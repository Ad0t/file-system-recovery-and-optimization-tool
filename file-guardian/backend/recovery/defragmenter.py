...
class Defragmenter:
                ...
                if isinstance(old_blocks, list) and len(old_blocks) > 1:
                    ...
                    if new_start is not None:
                        ...
                        if self._copy_blocks(old_blocks, new_blocks):
                            ...
                            # Update FAT: file_to_blocks AND block_to_file
                            self.fat.file_to_blocks[inode_number] = new_blocks
                            # Remove old block ownership
                            for b in old_blocks:
                                self.fat.block_to_file.pop(b, None)
                            # Add new block ownership
                            for b in new_blocks:
                                self.fat.block_to_file[b] = inode_number
    ...
    def compact_free_space(self) -> Dict[str, Any]:
        ...
        Reserved blocks (0-3) are preserved.
        """
        start_time = time.time()
        moved_files = 0
        blocks_moved = 0
        ...
        try:
            if self.fat and hasattr(self.fat, 'file_to_blocks'):
                ...
                current_free_pointer = 4  # Start after reserved blocks (0-3)
                ...
                for inode in sorted_inodes:
                    ...
                    if blocks[0] != current_free_pointer:
                        ...
                        if self._copy_blocks(blocks, new_blocks):
                            ...
                            # Remove old block ownership
                            for b in blocks:
                                if self.fat.block_to_file.get(b) == inode:
                                    self.fat.block_to_file.pop(b, None)
                            # Add new block ownership
                            for b in new_blocks:
                ...
                # Rebuild block ownership map once after all moves. This avoids
                # stale/deleted mappings when a later file's old blocks overlap
                # an earlier file's newly assigned destination blocks.
                self._rebuild_block_ownership_map()

                if self.fsm and hasattr(self.fsm, 'bitmap'):
                    # Preserve reserved blocks, mark used blocks, rest are free
                    self.fsm.bitmap.setall(0)
                    # Mark reserved blocks as used
                    for i in range(4):
                        self.fsm.bitmap[i] = 1
                    # Mark file blocks as used
                    for i in range(4, current_free_pointer):
                        self.fsm.bitmap[i] = 1
        except Exception as e:
    ...
    def _rebuild_block_ownership_map(self) -> None:
        """Recompute FAT block_to_file from current file_to_blocks mapping."""
        if not self.fat or not hasattr(self.fat, 'file_to_blocks') or not hasattr(self.fat, 'block_to_file'):
            return
    ...
    def get_defragmentation_plan(self, inode_numbers: List[int]) -> Dict[str, Any]:
        ...
        try:
            if self.fat and hasattr(self.fat, 'file_to_blocks'):
                sim_pointer = 0
                if self.fsm and hasattr(self.fsm, 'bitmap'):
                    sim_pointer = (getattr(self.fsm, 'total_blocks', getattr(self.disk, 'total_blocks', 1024)) - self.fsm.get_free_count()) + 1
    ...
    def rollback_defragmentation(self, operation_id: int) -> bool:
        ...
        try:
            ...
            if self._copy_blocks(new_b, old_b):
                if self.fat and hasattr(self.fat, 'file_to_blocks'):
                    # Restore file_to_blocks AND block_to_file
                    self.fat.file_to_blocks[inode] = old_b
                    # Remove new block ownership
                    for b in new_b:
                        self.fat.block_to_file.pop(b, None)
                    # Restore old block ownership
                    for b in old_b:
                        self.fat.block_to_file[b] = inode
    ...
    def _find_contiguous_space(self, num_blocks: int) -> Optional[int]:
        ...
        try:
            if self.fsm and hasattr(self.fsm, 'bitmap'):
                ...
                for b in range(RESERVED_BLOCK_COUNT, total_blocks):
    ...
    def _update_file_pointers(self, inode: Any, new_blocks: List[int]) -> bool:
        ...
        try:
            ...
            # Current inode shape in this project: direct/single/double pointers.
            if hasattr(inode, 'direct_pointers'):
                max_direct = len(getattr(inode, 'direct_pointers', []))
                if max_direct <= 0:
                    # Fall back if constant is not present on object.
                    max_direct = 12

                # Reset existing pointers
                inode.direct_pointers = []
                if hasattr(inode, 'single_indirect'):
                    inode.single_indirect = None
                if hasattr(inode, 'double_indirect'):
                    inode.double_indirect = None
                if hasattr(inode, 'indirect_blocks_data'):
                    inode.indirect_blocks_data["single"] = []
                    inode.indirect_blocks_data["double"] = {}

                direct = list(new_blocks[:max_direct])
                inode.direct_pointers.extend(direct)

                remaining = list(new_blocks[max_direct:])
                if remaining:
                    # Keep pointer data coherent even if we do not allocate real
                    # metadata blocks for the simulation.
                    inode.indirect_blocks_data["single"] = remaining

                if hasattr(inode, 'block_count'):
                    inode.block_count = len(new_blocks)
                if hasattr(inode, 'update_modified_time'):
            ...
            if self.fat and hasattr(self.fat, 'file_to_blocks') and inode_number in self.fat.file_to_blocks:
                ...
                if isinstance(old_blocks, list) and len(old_blocks) > 1:
                    ...
                    if new_start is not None:
                        ...
                        if self._copy_blocks(old_blocks, new_blocks):
                            # Atomic swap mock - update FAT: file_to_blocks AND block_to_file
                            self.fat.file_to_blocks[inode_number] = new_blocks
                            # Remove old block ownership
                            for b in old_blocks:
                                self.fat.block_to_file.pop(b, None)
